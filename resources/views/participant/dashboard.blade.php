    ...
    <div class="p-4 bg-gray-50 rounded-2xl m-2 mt-0 space-y-4 overflow-y-auto min-h-screen">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div class="mt-3 flex items-center gap-2">
                    <span class="text-xs text-blue-600 font-medium" id="stat-total-meetings-text">
            ...
            <div class="mt-3 flex items-center gap-2">
                    ...
                    <span class="text-xs text-purple-600 font-medium" id="stat-upcoming-meetings-text">
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            @forelse($schedule as $meeting)
                ...
                <div
                    ...
                    data-schedule-meeting-id="{{ $meeting->id }}"
                ...
                >
                    ...
                    <div class="flex items-center gap-2 min-w-0">
                        ...
                        <span class="relative flex h-2.5 w-2.5 flex-shrink-0" id="schedule-dot-{{ $meeting->id }}">
                    ...
                    <!-- Action -->
                    <div class="text-right" id="schedule-action-{{ $meeting->id }}">
...
{{-- ================================================================
     LIVE STATUS SYNC — exact server time, no manual page refresh.
================================================================ --}}
<script>
    (function () {
        const scheduleRows = Array.from(document.querySelectorAll('[data-schedule-meeting-id]'));
        const scheduleIds = [...new Set(scheduleRows.map(row => row.dataset.scheduleMeetingId))];
        ...
        function updateStatCards(stats) {
            ...
            const totalEl = document.getElementById('stat-total-meetings-text');
            if (totalEl && stats.total !== undefined) {
                totalEl.textContent = `${stats.total} ${stats.total === 1 ? 'meeting' : 'meetings'}`;
            }

            const todayCardEl = document.getElementById('today-card-count');
            if (todayCardEl && stats.active !== undefined && stats.today !== undefined) {
                todayCardEl.innerHTML = stats.active > 0
                    ? `<span class="text-xs text-emerald-600 font-medium flex items-center gap-1">
                        <span class="relative flex h-2 w-2">
                            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                            <span class="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                    ...
                    : `<span class="text-xs text-gray-500 font-medium">${stats.today} today</span>`;
            }

            const upcomingEl = document.getElementById('stat-upcoming-meetings-text');
            if (upcomingEl && stats.upcoming !== undefined) {
                upcomingEl.textContent = `${stats.upcoming} upcoming`;
            }
        }

        function scheduleDotHtml(isLive) {
            return `<span class="relative flex h-2.5 w-2.5 flex-shrink-0">
                ${isLive ? '<span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-75"></span>' : ''}
                <span class="relative inline-flex rounded-full h-2.5 w-2.5 ${isLive ? 'bg-blue-600' : 'bg-gray-300'}"></span>
            </span>`;
        }

        function scheduleActionHtml(status, id) {
            if (status === 'active') {
                return `<a href="/participant/meetings/${id}/attend" class="inline-block px-5 py-2 rounded-lg text-xs font-semibold transition bg-gradient-to-r from-blue-600 to-indigo-600 text-white hover:from-blue-700 hover:to-indigo-700 shadow-sm hover:shadow">Join Session</a>`;
            }
            return `<span class="inline-block px-5 py-2 rounded-lg text-xs font-semibold bg-gray-100 text-gray-600 cursor-not-allowed">Attend Meeting</span>`;
        }
        ...
        async function poll(reason = 'manual') {
            ...
            try {
                ...
                url.searchParams.set('ids', scheduleIds.join(','));
                ...
                if (!response.ok) {
                    throw new Error(`Dashboard status refresh failed with HTTP ${response.status}`);
                }
                ...
                Object.entries(data.meetings || {}).forEach(([id, status]) => {
                    const row = document.querySelector(`[data-schedule-meeting-id="${id}"]`);
                    ...
                    const dot = document.getElementById('schedule-dot-' + id);
                    if (dot) {
                        dot.outerHTML = scheduleDotHtml(status === 'active');
                    }

                    const action = document.getElementById('schedule-action-' + id);
                    if (action) {
                        action.innerHTML = scheduleActionHtml(status, id);
                    }
                });

                scheduleExactRefresh();
            } catch (error) {
                console.error(`Dashboard status refresh failed (${reason}):`, error);
            } finally {
        ...
        // Stats (total/live/upcoming/today) still need refreshing even when
        // the near-term schedule table is empty, so always poll at least once.
        poll('initial-load');
