...
<script id="sm-participant-outbound-camera-fix">
    /* ================================================================
       PARTICIPANT -> ORGANIZER OUTBOUND CAMERA REPAIR
       Keeps the existing fixed sendrecv transceivers and TURN config.
       No extra m-lines are added.
    ================================================================ */
    (() => {
        async function forceParticipantVideoToPeers() {
            ...
            for (const uid of ids) {
                ...
                let sender = pc.__smVideoSender || tx?.sender ||
                    pc.getSenders?.().find(s => s.track?.kind === 'video');

                if (!sender) {
                    console.warn('SmartMeet: video sender missing for peer', uid);
                    continue;
                }
                ...
                try {
                    ...
                    pc.__smVideoSender = sender;
                } catch (e) {
                    console.warn('SmartMeet participant video replaceTrack failed:', uid, e);
                }

                /* Make sure an existing encoding was not left inactive. */
                try {
        ...
        const originalToggle = window.toggleCamera;

        if (typeof originalToggle === 'function' && !window.__smParticipantCameraWrapped) {
            window.__smParticipantCameraWrapped = true;

            window.toggleCamera = async function (...args) {
                try {
                    await originalToggle.apply(this, args);
                } catch (error) {
                ...
                /* Re-install the same live track after browser permission settles. */
                [80, 280, 750, 1500].forEach(delay => {
                    setTimeout(() => {
                        forceParticipantVideoToPeers().catch(console.warn);
                    }, delay);
                });
            };
        ...
        /* Also repair after join / camera-state signaling. */
        if (window.Echo && typeof ROOM !== 'undefined' && !window.__smParticipantOutboundBound) {
            window.__smParticipantOutboundBound = true;

            window.Echo.channel(ROOM).listen('.signal', event => {
                if (event?.type === 'user-joined' || event?.type === 'camera-status') {
                    if (isCameraOn) {
                        setTimeout(() => forceParticipantVideoToPeers().catch(() => {}), 160);
                        setTimeout(() => forceParticipantVideoToPeers().catch(() => {}), 700);
                    }
                }
            });
        }
        ...
        window.addEventListener('load', () => {
            if (isCameraOn) {
                setTimeout(() => forceParticipantVideoToPeers().catch(() => {}), 500);
            }
        });
    })();
</script>
