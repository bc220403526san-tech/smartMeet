...
<x-layouts.app>
    ...
    <div class="p-4 bg-gray-50 rounded-2xl m-2 mt-0 space-y-4 overflow-y-auto min-h-screen">
        {{-- Global flash banner --}}
        @if (session('success'))
            <div id="flash-success" class="flex items-start justify-between gap-3 rounded-xl bg-green-50 border border-green-200 text-green-700 text-sm px-4 py-3 transition-all duration-500">
                <span>{{ session('success') }}</span>
                <button type="button" onclick="document.getElementById('flash-success').remove()"
                        class="shrink-0 text-green-500 hover:text-green-700 transition-colors">
        ...
        @endif
        @if ($errors->any())
            <div id="flash-error" class="flex items-start justify-between gap-3 rounded-xl bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-3 transition-all duration-500">
                    ...
                    <div class="flex flex-col items-center -mt-10 sm:-mt-12 px-4 pb-4">
                        ...
                        <div class="relative group mb-2 w-16 h-16 sm:w-20 sm:h-20">
                            ...
                            <button type="button" id="avatar-trigger"
                                    class="absolute -bottom-1 -right-1 w-7 h-7 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 border-2 border-white rounded-full flex items-center justify-center shadow-md cursor-pointer transition-colors duration-150">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 text-white pointer-events-none" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
            ...
            <div class="lg:col-span-8">
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                    ...
                    <div class="md:col-span-8 bg-white rounded-2xl shadow-md border border-gray-100 hover:shadow-xl transition-all duration-300">
                        ...
                        <form action="{{ route('participant.settings.profile.update') }}" method="POST" class="p-4">
                            @csrf
                            @method('PATCH')
                    ...
                    <div class="md:col-span-4 bg-white rounded-2xl shadow-md border border-gray-100 hover:shadow-xl transition-all duration-300">
                        ...
                        <div class="p-4 space-y-2.5">
                            @php
                                $toggles = [
                                    ['key' => 'notif_email', 'label' => 'Email Alerts', 'desc' => 'Get notified via email'],
                                    ['key' => 'notif_meeting_reminders', 'label' => 'Reminders', 'desc' => 'Meeting & task reminders'],
                                    ['key' => 'notif_sound', 'label' => 'Sound Alerts', 'desc' => 'Play sound on new alerts'],
                                ];
                            @endphp
        ...
        <div class="w-full bg-white rounded-2xl shadow-md border border-gray-100 hover:shadow-xl transition-all duration-300 overflow-hidden">
            <div class="flex items-center gap-3 px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400">Request a role upgrade to unlock more features</p>
                </div>
            </div>
            <div class="p-4 sm:p-6">
                ...
                <div id="role-request-modal" class="hidden fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24 px-4">
                    ...
                    <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden animate-[fadeIn_.15s_ease-out]">
                        ...
                        <form id="role-request-form" method="POST" action="{{ route('participant.settings.role-request') }}" class="p-5" onsubmit="return submitRoleRequest(event)">
                            @csrf
                            <div class="grid grid-cols-2 gap-3 mb-3 pb-3 border-b border-gray-100">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-700 font-medium">Participant</p>
                                </div>
                                <div>
                ...
                Organizer
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="subject" required placeholder="e.g. Request for Organizer Access"
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="message" required rows="4" placeholder="Explain why you'd like to become an Organizer..."
        ...
        <div class="w-full bg-white rounded-2xl shadow-md border border-gray-100 hover:shadow-xl transition-all duration-300 overflow-hidden">
            ...
            <form action="{{ route('participant.settings.password.update') }}" method="POST" class="p-4 sm:p-6">
                @csrf
                @method('PUT')
        ...
        <div class="w-full bg-gradient-to-r from-red-50 to-red-50/50 rounded-2xl border border-red-200 hover:border-red-300 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden">
            <form action="{{ route('participant.settings.deactivate') }}" method="POST"
    ...
    <script>
        (function () {
            ...
            window.submitRoleRequest = function (e) {
                ...
                if (!csrfToken) {
                    showToast('Missing CSRF meta tag.', 'error');
                    return false;
                }
                btn.disabled = true;
            ...
            <div class="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-xs text-amber-700 mt-1">
                        Your request was just submitted for Organizer access. An admin will review it shortly.
                    </p>
                </div>
            </div>`;
                }
            ...
            function showToast(message, type = 'success') {
                ...
                toast.className = `fixed top-4 right-4 z-[9999] ${colors} border text-sm px-4 py-3 rounded-xl shadow-lg transition-opacity duration-500`;
            ...
            function showFlashMessage(message, type = 'success') {
                fetch('{{ route("participant.settings.flash") }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken,
                        'Accept': 'application/json',
            ...
            function revertAllAvatarsToInitials() {
                document.querySelectorAll('.app-avatar-img').forEach((img) => { img.style.display = 'none'; });
                document.querySelectorAll('.app-avatar-initials').forEach((el) => { el.style.display = ''; });
            }
            ...
            function uploadAvatar(file) {
                ...
                fetch('{{ route("participant.settings.avatar.update") }}', {
                    method: 'POST',
            ...
            // ---- Avatar: click badge -> open file picker (delegated) ----
            document.addEventListener('click', function (e) {
                const trigger = e.target.closest('#avatar-trigger');
                if (!trigger) return;
                const input = document.getElementById('avatar-input');
                if (!input) {
                    console.error('[settings] #avatar-input not found in the DOM.');
                    return;
                }
                input.click();
            });

            // ---- Avatar: file chosen (delegated) ----
            ...
            // ---- Notification toggles (delegated) ----
            document.addEventListener('change', function (e) {
                ...
                fetch('{{ route("participant.settings.notifications.update") }}', {
                    method: 'PATCH',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken,
                        'Accept': 'application/json',
            ...
            if (!document.getElementById('avatar-trigger')) console.error('[settings] #avatar-trigger badge is missing from the page.');
            if (!document.getElementById('avatar-input')) console.error('[settings] #avatar-input file input is missing from the page.');
        })();
    </script>
