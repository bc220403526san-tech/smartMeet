<x-layouts.app>
    ...
    <div class="p-3 sm:p-4 bg-gray-50 rounded-2xl m-2 mt-0 space-y-4 overflow-y-auto min-h-screen">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4 sm:mb-6">
            <div class="min-w-0">
                <h1 class="text-lg sm:text-xl md:text-2xl font-semibold text-gray-800">Role Change Requests</h1>
            ...
            @if($requests->where('status', 'pending')->count() > 0)
                {{-- Visible on all screen sizes now, not just sm+ --}}
                <span class="inline-flex items-center gap-1.5 bg-amber-50 text-amber-700 text-xs font-medium px-3 py-1.5 rounded-full border border-amber-200 self-start sm:self-auto whitespace-nowrap">
                ...
                <table class="w-full text-sm min-w-[720px]">
                    ...
                    @forelse($requests as $req)
                        <tr class="hover:bg-gray-50/60 transition-colors duration-150">
                            ...
                            {{-- Requested role --}}
                            <td class="px-4">
                                <span class="inline-flex items-center gap-1.5 bg-indigo-50 text-indigo-700 text-xs font-medium px-2.5 py-1 rounded-full whitespace-nowrap">
                            ...
                            <td class="px-4">
                                ...
                                @endphp
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium whitespace-nowrap {{ $statusStyles[$req->status] ?? 'bg-gray-100 text-gray-600' }}">
                                    <span class="w-1.5 h-1.5 rounded-full shrink-0 {{ $dotStyles[$req->status] ?? 'bg-gray-400' }}"></span>
                                    {{ ucfirst($req->status) }}
                                </span>
                            </td>
                            {{-- Actions --}}
                            <td class="px-4">
                                ...
                                @else
                                    <div class="flex items-center justify-end gap-3">
                                        <p class="text-xs text-gray-400 italic whitespace-nowrap">Already reviewed</p>
                ...
                @forelse($requests as $req)
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex items-center gap-3 min-w-0 mb-3">
                            <div class="w-9 h-9 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center text-xs font-semibold shrink-0">
                                {{ strtoupper(substr($req->user->name, 0, 1)) }}
                            </div>
                            <div class="min-w-0 flex-1">
                                <p class="text-gray-800 font-medium text-sm truncate">{{ $req->user->name }}</p>
                                <p class="text-xs text-gray-400 truncate">{{ $req->user->email }}</p>
                            </div>
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium shrink-0 whitespace-nowrap {{ $statusStyles[$req->status] ?? 'bg-gray-100 text-gray-600' }}">
                                <span class="w-1.5 h-1.5 rounded-full shrink-0 {{ $dotStyles[$req->status] ?? 'bg-gray-400' }}"></span>
                                {{ ucfirst($req->status) }}
                            </span>
                        </div>

                        {{-- Subject + requested role --}}
                        <div class="flex flex-wrap items-center gap-2 mb-3 text-xs">
                            <span class="text-gray-500">Subject:</span>
                            <span class="text-gray-700">{{ $req->subject }}</span>
                            <span class="text-gray-300">|</span>
                            <span class="inline-flex items-center gap-1.5 bg-indigo-50 text-indigo-700 font-medium px-2.5 py-1 rounded-full whitespace-nowrap">
                                {{ ucfirst($req->requested_role) }}
                            </span>
                        </div>
                        ...
                        @if($req->status === 'pending')
                            <div class="flex items-center gap-2">
                                <form method="POST" action="{{ route('admin.role-requests.approve', $req) }}" class="flex-1">
                                    @csrf @method('PATCH')
                                    <button type="submit"
                                            class="w-full inline-flex items-center justify-center gap-1 text-xs font-medium text-emerald-700 bg-emerald-50 hover:bg-emerald-100 px-3 py-2 rounded-lg transition-colors">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor">
                                            <path fill-rule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clip-rule="evenodd" />
                                        </svg>
                                        Approve
                                ...
                                <form method="POST" action="{{ route('admin.role-requests.reject', $req) }}" class="flex-1">
                                    @csrf @method('PATCH')
                                    <button type="submit"
                                            class="w-full inline-flex items-center justify-center gap-1 text-xs font-medium text-rose-700 bg-rose-50 hover:bg-rose-100 px-3 py-2 rounded-lg transition-colors">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor">
                                            <path fill-rule="evenodd" d="M10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 011.414-1.414L10 8.586z" clip-rule="evenodd" />
                                        </svg>
                                        Reject
        ...
        @if($requests->hasPages())
            {{-- Wrapped in overflow-x-auto so default Laravel pagination links
                 don't break the layout on very narrow screens --}}
