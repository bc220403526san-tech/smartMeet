    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .tile-empty-state {
            ...
            color: var(--text-faint); padding: 30px; text-align: center;
        }
        .tile-empty-state i { font-size: 26px; opacity: .6; }
        ...
        .modal-card {
            background: #11162457; background: linear-gradient(160deg, #171d30, #0e1220);
...
<div class="app-shell">
    ...
    <header class="topbar">
        ...
        <div class="topbar-right">
            ...
            <button class="btn danger" id="cancel-meeting-btn">
                <i class="fa-solid fa-ban"></i> End meeting for everyone
            </button>
        </div>
    </header>
...
<div class="modal-overlay" id="cancelled-modal">
    <div class="modal-card">
        ...
        <h3>Meeting ended</h3>
        <p>You have ended this meeting for all participants. Redirecting…</p>
    </div>
</div>
...
<script>
    /* ============================================================
       SMARTMEET — ORGANIZER ROOM
       Single, consistent WebRTC mesh + Reverb signaling implementation.
    ============================================================ */
    (function () {
        'use strict';
        ...
        const MY_INITIALS = @json($orgInitials);
        ...
        // ---------- State ----------
        const people = {};          // userId -> { name, initials, isOrganizer, joined:boolean }
        const peers = {};           // userId -> RTCPeerConnection
        const pendingCandidates = {}; // userId -> [candidate,...] queued before remoteDescription is set
        ...
        people[MY_USER_ID] = { name: MY_NAME, initials: MY_INITIALS, isOrganizer: true, joined: true };
        ...
        // ============================================================
        // VIDEO TILES  — only ever rendered for users who have actually joined
        // ============================================================
        ...
        async function handleSignal(payload) {
            ...
            if (!type || !fromUserId || String(fromUserId) === String(MY_USER_ID)) return;
            ...
            switch (type) {
                case 'user-joined': {
                    ...
                    setPersonJoined(uid, true, { name: data.name, initials: data.initials || initials(data.name), isOrganizer: !!data.isOrganizer });
                    connectTo(uid);
                ...
                case 'chat':
                    appendChatMessage({ userId: fromUserId, name: data.name, initials: data.initials || initials(data.name), text: data.text }, false);
                    break;
                case 'mic-status':
                    setMicIndicator(String(data.userId), !!data.muted);
                    break;
                case 'camera-status':
                    // remote camera-off state is reflected automatically once their track disables/mutes
                    break;
                case 'meeting-cancelled':
                    // Organizer triggers this themselves; nothing to do here.
                    break;
            }
        }
        ...
        $('#cancel-meeting-btn').addEventListener('click', async () => {
            if (!confirm('End this meeting for everyone? All participants will be disconnected.')) return;
            $('#cancelled-modal').classList.add('active');
            await sendSignal('all', 'meeting-cancelled', { by: MY_NAME });
            teardown();
            setTimeout(() => $('#cancel-form').submit(), 600);
        });
        ...
        // sending, leave, cancel) still works — you just won't see other
        // people join in realtime until Echo is fixed.
        // ============================================================
        try {
            if (!window.Echo) {
